...
namespace Kedy {
	...
	void PieceTable::DeleteText(int pos, int length) {
		int currentPos = 0;
		for (size_t i = 0; i < m_Pieces.size(); ++i) {
			Piece& piece = m_Pieces[i];
			if (currentPos <= pos && pos < currentPos + piece.length) {
	...
	void PieceTable::SplitPiece(int pieceIndex, int pos) {
